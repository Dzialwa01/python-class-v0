...
# client.chat.completions -> This tells OpenAI API that you want to use the chat completion system - meaning I am sending a coversation with roles like: system, user and assistant.
# .create means that create a new response from the model
response = client.chat.completions.create(
    model="gpt-4o-mini", # indicates the model that should generate the answer (the brains the bot will use)
...
# response.choices[0].message.content -> response contains everything the API returned, including: the generated answer, metadata, token usage, message structure.
## The API stores generated outputs inside a list called choices. Usually there is only one response, so we access the first item [0]
## `inside the choice, there is a message object that contains: role, content`. .content is the acual text answer generated by the AI
